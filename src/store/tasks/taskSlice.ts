...
export const fetchTasks = createAsyncThunk("tasks/fetchTasks", async () => {
  const response = await axiosInstance.get("/tasks?_embed=comments&_embed=user");
...
export const fetchTasksForPagination = createAsyncThunk("tasks/fetchTasksForPagination", async (params: FetchTasksParams) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTask.fulfilled, (state, action) => {
        ...
        const indexPag = state.paginationTasks.findIndex(task => task.id === action.payload.id);
        state.paginationTasks[indexPag] = action.payload;
      })
      .addCase(deleteTask.fulfilled, (state, action) => {
        state.tasks = state.tasks.filter(task => task.id !== action.payload);
        state.paginationTasks = state.paginationTasks.filter(task => task.id !== action.payload);
      })
